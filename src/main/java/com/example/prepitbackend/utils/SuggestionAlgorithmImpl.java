...
@AllArgsConstructor
public class SuggestionAlgorithmImpl implements SuggestionAlgorithm{
    ...
    public SuggestionAlgorithmImpl(ArrayList<MealDTO> meals, String goal, Double priceMultiplier, Double timeMultiplier, Double caloriesMultiplier){
        this.meals = meals;
        this.goal = goal;
    ...
    public ArrayList<ArrayList<MealDTO>> suggestRecipesByIngredients(ArrayList<String> targets){
        // target have a list with ingredients, search through recipes and check for which recipes to suggest
        // use calculate distance, try to rewrite it so it works
        //  since the wight == 0 means that the recipe has all the wanted ingredients, we need a min heap instead of a max heap
        return new ArrayList<ArrayList<MealDTO>>();
    }
    ...
    public ArrayList<MealDTO> getKclosest(MealAlgorithmDTO target, int k) {
        ...
        // Make a max heap.
        PriorityQueue<Pair<MealDTO, Double>> pq = createMaxHeap();
        ...
        while(number < k && index < this.meals.size()){
            ...
            if((chooseBasedOnGoal(candidate, target)) && (!candidate.getType().equals("dessert"))){
     ...
     * Note: The "NONE" value is used when the user wants only meals containing certain ingredients regardless of their goal
